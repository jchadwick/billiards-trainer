...
class GeometryUtils:
    ...
    def create_regular_polygon(
        center: Point2D, radius: float, sides: int
    ) -> list[Point2D]:
    ...
    def simplify_polygon(
        polygon: list[Point2D], tolerance: float = 1.0
    ) -> list[Point2D]:
        ...
        def douglas_peucker(
            points: list[Point2D], start_idx: int, end_idx: int
        ) -> list[Point2D]:
            ...
            if max_distance > tolerance:
                ...
                left_part = douglas_peucker(points, start_idx, max_idx)
                right_part = douglas_peucker(points, max_idx, end_idx)
                return left_part[:-1] + right_part  # Remove duplicate point
            else:
                return [points[start_idx], points[end_idx]]
        ...
        if polygon[0].distance_to(polygon[-1]) > tolerance:
            polygon = polygon + [polygon[0]]

        simplified = douglas_peucker(polygon, 0, len(polygon) - 1)

        # Remove closing point if added
        if (
            len(simplified) > 1
            and simplified[0].distance_to(simplified[-1]) < tolerance
        ):
