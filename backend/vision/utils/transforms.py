    ...
    def __post_init__(self):
        ...
        if self.is_homogeneous:
            if self.matrix.shape not in [(3, 3), (4, 4)]:
                raise ValueError(f"Homogeneous matrix must be 3x3 or 4x4, got {self.matrix.shape}")
        else:
            if self.matrix.shape[1] != self.matrix.shape[0]:
                raise ValueError(f"Transformation matrix must be square, got {self.matrix.shape}")
...
class CoordinateTransformer:
    ...
    def __init__(self):
        ...
        self.transformations: dict[Tuple[CoordinateSystem, CoordinateSystem], TransformationMatrix] = {}
    ...
    def set_camera_calibration(self, calibration: CameraCalibration) -> None:
        ...
        logger.info(f"Camera calibration set: {calibration.image_size}, error={calibration.reprojection_error:.3f}")
    ...
    def add_transformation(self, transform: TransformationMatrix) -> None:
        ...
        logger.debug(f"Added transformation: {transform.source_system.value} -> {transform.target_system.value}")
    ...
    def transform_point(
        ...
        if isinstance(point, tuple):
            if len(point) == 2:
                ...
                raise ValueError(f"Point tuple must have 2 or 3 elements, got {len(point)}")
        ...
        if inverse_key in self.transformations:
            return self._apply_transformation(point, self.transformations[inverse_key].inverse)
        ...
        if path:
            ...
            for i in range(len(path) - 1):
                current_point = self.transform_point(current_point, path[i], path[i + 1])
            return current_point

        raise ValueError(f"No transformation available from {source_system.value} to {target_system.value}")
    ...
    def compute_perspective_transform(
        ...
        if len(source_points) < 4 or len(target_points) < 4:
            raise ValueError("At least 4 point pairs required for perspective transform")
    ...
    def _compute_table_homography(self) -> None:
        ...
        homography = self.compute_perspective_transform(
            self.table_corners_camera,
            self.table_corners_world
        )
    ...
    def get_available_transformations(self) -> List[Tuple[CoordinateSystem, CoordinateSystem]]:
