...
class SchemaValidator:
    ...
    def validate_camera(self, camera_config: dict) -> None:
        ...
        """
        is_valid, errors = self._validate_with_pydantic_model(
            camera_config, CameraSettings
        )
        if not is_valid:
            raise ValueError(
                f"Camera configuration validation failed: {'; '.join(errors)}"
            )

    def validate_table(self, table_config: dict) -> None:
        ...
        Raises:
            ValueError: If validation fails
        """
        # Table validation using basic JSON schema approach
        # since there's no dedicated TableConfig in the Pydantic models
        table_schema = {
            "width": {"type": "number", "minimum": 1.0, "maximum": 5.0},
            "height": {"type": "number", "minimum": 0.5, "maximum": 3.0},
            "pocket_radius": {"type": "number", "minimum": 0.01, "maximum": 0.1},
        }

        is_valid, errors = self._validate_with_json_schema(table_config, table_schema)
        if not is_valid:
            raise ValueError(
                f"Table configuration validation failed: {'; '.join(errors)}"
            )

    def validate_physics(self, physics_config: dict) -> None:
        ...
        Raises:
            ValueError: If validation fails
        """
        # Physics validation using basic JSON schema approach
        physics_schema = {
            "friction": {"type": "number", "minimum": 0.0, "maximum": 1.0},
            "restitution": {"type": "number", "minimum": 0.0, "maximum": 1.0},
            "gravity": {"type": "number", "minimum": 0.0, "maximum": 50.0},
        }

        is_valid, errors = self._validate_with_json_schema(
            physics_config, physics_schema
        )
        if not is_valid:
            raise ValueError(
                f"Physics configuration validation failed: {'; '.join(errors)}"
            )
