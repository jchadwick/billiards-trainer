...
export const NetworkDiagnostics: React.FC<NetworkDiagnosticsProps> = observer(({ suites }) => {
  ...
  const testEndpoint = useCallback(async (endpointId: string): Promise<void> => {
      ...
      if (endpoint.type === 'websocket') {
        ...
        setEndpoints(prev => prev.map(ep =>
          ep.id === endpointId ? {
            ...
            errorCount: response.ok ? ep.errorCount : ep.errorCount + 1,
            uptime: response.ok ? ep.uptime + 1 : ep.uptime,
          } : ep
        ));
  ...
  const runBandwidthTest = useCallback(async (testId: string): Promise<void> => {
    setBandwidthTests(prev => prev.map(test =>
  ...
  const runDownloadTest = useCallback(async (testId: string): Promise<void> => {
    ...
    for (let i = 0; i < testSizes.length; i++) {
      ...
      // Simulate download by requesting a test file
      const response = await fetch(`/api/test/download/${size}mb`);
      ...
      setBandwidthTests(prev => prev.map(test =>
        test.id === testId ? { ...test, progress: ((i + 1) / testSizes.length) * 100 } : test
      ));
    }
  ...
  const runPingTest = useCallback(async (testId: string): Promise<void> => {
    ...
    for (let i = 0; i < pingCount; i++) {
      ...
      setBandwidthTests(prev => prev.map(test =>
        test.id === testId ? { ...test, progress: ((i + 1) / pingCount) * 100 } : test
      ));
    ...
    const packetLoss = (latencies.filter(lat => lat === 999).length / latencies.length) * 100;
  ...
  const getTestStatusColor = (status: BandwidthTest['status']): 'green' | 'yellow' | 'red' | 'blue' | 'gray' => {
    switch (status) {
  ...
  const onlineEndpoints = endpoints.filter(ep => ep.status === 'online').length;
  const avgLatency = latencyHistory.length > 0 ?
    latencyHistory[latencyHistory.length - 1].value : 0;
  const avgThroughput = throughputHistory.length > 0 ?
    throughputHistory[throughputHistory.length - 1].value : 0;
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <StatCard
          ...
          value={connectionStore.state.isConnected ? 'Connected' : 'Disconnected'}
          icon="🌐"
        ...
        <StatCard
          ...
          unit="ms"
          icon="⚡"
          color={avgLatency < 100 ? 'green' : avgLatency < 300 ? 'yellow' : 'red'}
        />
        <StatCard
          title="Throughput"
      ...
      {networkInfo && (
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
            <div>
              <span className="text-gray-500 dark:text-gray-400">Connection Type:</span>
              <span className="ml-2 text-gray-900 dark:text-white">{networkInfo.type}</span>
            </div>
            <div>
              <span className="text-gray-500 dark:text-gray-400">Effective Type:</span>
              <span className="ml-2 text-gray-900 dark:text-white">{networkInfo.effectiveType}</span>
            </div>
            <div>
              <span className="text-gray-500 dark:text-gray-400">Online Status:</span>
              <span className="ml-2 text-gray-900 dark:text-white">{networkInfo.online ? 'Online' : 'Offline'}</span>
            </div>
          </div>
        </div>
      )}
                ...
                <div className="space-y-2 text-sm">
                  {test.result.speed > 0 && (
                    <div className="flex justify-between">
                      ...
                      <span className="text-gray-900 dark:text-white">{test.result.speed.toFixed(1)} Mbps</span>
                    </div>
                  )}
                  {test.result.latency > 0 && (
                    <div className="flex justify-between">
                      <span className="text-gray-500 dark:text-gray-400">Latency:</span>
                      <span className="text-gray-900 dark:text-white">{test.result.latency.toFixed(1)} ms</span>
                    </div>
                  )}
                  {test.result.packetLoss > 0 && (
                    <div className="flex justify-between">
                      <span className="text-gray-500 dark:text-gray-400">Packet Loss:</span>
                      <span className="text-gray-900 dark:text-white">{test.result.packetLoss.toFixed(1)}%</span>
                    </div>
                  )}
                </div>
              )}
              ...
              <div className="flex items-center space-x-4">
                ...
                <div>
                  ...
                  <div className="flex items-center space-x-4 mt-1 text-xs text-gray-500 dark:text-gray-400">
                    <span>Response: {endpoint.responseTime.toFixed(0)}ms</span>
                    <span>Errors: {endpoint.errorCount}</span>
                    {endpoint.statusCode && <span>Status: {endpoint.statusCode}</span>}
                  </div>
                </div>
              </div>
